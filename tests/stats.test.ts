...
describe('Stats Service', () => {
  it('should aggregate product quantities', async () => {
    ...
    await insertOrder('ORD-1', 'test1@example.com', JSON.stringify([{ price_id: 'price_a', quantity: 2 }]));
    await insertOrder('ORD-2', 'test2@example.com', JSON.stringify([{ price_id: 'price_a', quantity: 1 }]));
    await insertOrder('ORD-3', 'test3@example.com', JSON.stringify([{ price_id: 'price_b', quantity: 3 }]));
